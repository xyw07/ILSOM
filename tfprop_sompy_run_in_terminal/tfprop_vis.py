...
from utils.strings import str_wrap_chem as str_wrap
HUGE = 10000000
...
def show_posmap(som, placement_name_df, raw_name_df, n_clusters, cl_labels,
                show_data=True, labels=False, isOutPosmap=True, isOutKmeans=True):
    ...
    # save figure of k means
    if isOutKmeans:
        print("Saving figure of SOM k means map to {}...".
              format(tfpinit.fout_kmeans))
        fig.savefig(tfpinit.fout_kmeans)
...
# We may want to move to python's typing system later
    def show(self, som :sompy.sompy.SOM, cl_labels :list, what='codebook', which_dim='all',
             ...
             isOutHtmap=True):# it was orginally False by xinyuewang
...
class UMatrixTFP(UMatrixView):
    def show(self, som, placement_name_df, raw_name_df, savepath,
             distance2=1, row_normalized=False, show_data=False,
             contooor=True, blob=False, labels=False, cmap=None,
             isOutUmat=True): # change to True by xinyuewang
...
def main():
    ...
    tfpsom.sm.codebook.matrix = codemat_df.to_numpy()
    ...
    if tfpinit.isExeKmean:
        ...
        show_posmap(tfpsom.sm, tfpsom.fluid_name_df, tfpsom.fluid_name_df, #commented by xinyuewang
                    tfpsom.km_cluster, cl_labels,
                    show_data=False, labels=False, isOutPosmap=False, isOutKmeans=True)
        # show_posmap(tfpsom.sm, tfpinit.input_tfprop[1], tfpinit.input_tfprop[1], # xinyuewang's version
        #             tfpsom.km_cluster, cl_labels,
        #             show_data=True, labels=False)
    ...
    if tfpinit.isExePot:
        ...
        show_posmap(tfpsom.sm, tfpsom.fluid_name_df, tfpsom.fluid_name_df,
                    tfpsom.km_cluster, cl_labels,
                    show_data=False, labels=False,isOutPosmap=True, isOutKmeans=False)
    ...
    if tfpinit.isExeHtmap:
        ...
        viewTFP.show(tfpsom.sm, cl_labels, col_sz=tfpinit.heatmap_col_sz,
                     which_dim='all', desnormalize=True, col_norm='median',savepath=tfpinit.fout_htmap,# savepath by xinyuewang
                     cmap=cmap)
        # here is by xinyuewang
    # if tfpinit.isOutHtmap:
    #     print("Saving figure of heat map for all thermofluid prop. to {}...".format(tfpinit.fout_htmap))
    #     plt.figure(figsize=(tfpinit.heatmap_size[0], tfpinit.heatmap_size[1])).savefig(tfpinit.fout_htmap)
    ...
    if tfpinit.isExeUmat:
        ...
        umat = umatrixTFP.show(tfpsom.sm, tfpsom.fluid_name_df,
                               tfpsom.fluid_name_df,
                               'data_vis_largeset/umatrix.png',
                               show_data=False, labels=False, contooor=False,
                               cmap=cmap, isOutUmat=tfpinit.isOutUmat)
        #from cluster_inspector import make_cluster_graph
        #fig, ax = make_cluster_graph(tfpsom.sm, cl_labels)
        fig, ax = plt.subplots(1, 1, figsize=(30,30))
        clusteringmap_category(ax, tfpsom.sm,
                    tfpsom.km_cluster, tfpsom.fluid_name_df,
                    "Name", "Name", 'data_vis_largeset/cluster_category_plot.png')# "TYPE" was "NAME" originally by xinyuewang
